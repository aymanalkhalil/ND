...
if (isset($_POST['upload'])) {
    ...
    $path = 'assets/merchant_contest_uploads/';
    ...
    if (in_array(end($type), $allowed_types)) {
        ...
        $query = mysqli_query($conn, "INSERT INTO merchant_contest(merchant_id,upload_file)VALUES('{$_SESSION['merchant_id']}','$file_ext')");
        ...
        if ($query) {
         ...
         window.location.href = 'merchants-contest.php';
         }, 2000);</script>";
        } else {
...
if (isset($_POST['vote'])) {
    ...
    if ($_POST['type'] == 'merchant') {
        ...
        $votes = mysqli_query($conn, "SELECT votes from merchant_contest where m_contest_id='$contest_id'");
        $vote_row = mysqli_fetch_assoc($votes);
        $increment = $vote_row['votes'] + 1;
        mysqli_query($conn, "UPDATE merchant_contest SET votes='$increment' where m_contest_id='$contest_id'");

        $v_query = mysqli_query($conn, "INSERT INTO voters_active_contest(merchant_id,merchant_contest_id,voted)VALUES('{$_SESSION['merchant_id']}','$contest_id',1)");
        if ($v_query) {
        ...
        $v_query = mysqli_query($conn, "INSERT INTO voters_active_contest(sponsor_id,merchant_contest_id,voted)VALUES('{$_SESSION['sponsor_id']}','$contest_id',1)");
        if ($v_query) {
        ...
        $v_query = mysqli_query($conn, "INSERT INTO voters_active_contest(user_id,merchant_contest_id,voted)VALUES('{$_SESSION['user_id']}','$contest_id',1)");
        if ($v_query) {
        ...
        $v_query = mysqli_query($conn, "INSERT INTO voter_merchant_contest(voter_id,m_contest_id,voted)VALUES('{$_SESSION['voter_id']}','$contest_id',1)");
        if ($v_query) {
...
<section class="position-relative">
    ...
    <div class="container">
        <div class="row  text-center">
            <div class="col">
                <h1>مسابقات اصحاب المنتجات التجارية</h1>
        ...
        <div class="container">
            <div class="row">
                <div class="col-lg-9 col-md-12 order-lg-12">
                    <div class="row text-center">
                        <?php
                        $get_uploads = mysqli_query($conn, "SELECT m_contest_id,merchant_name,upload_file FROM merchants INNER JOIN merchant_contest
                               ON merchants.merchant_id=merchant_contest.merchant_id where active=1 ORDER BY m_contest_id DESC LIMIT $offset, $num_records_per_page ");
                        ...
                        <div class='container'>
                            ...
                            $total_merchant_contests = mysqli_query($conn, "SELECT * FROM merchant_contest");
                            $total_rows_s = mysqli_num_rows($total_merchant_contests);
                                    ...
                                    <div class="product-item">
                                        <div class="product-img">
                                            <?php if ($type == 'image') { ?>
                                                ...
                                                <a href="assets/merchant_contest_uploads/<?php echo $uploads['upload_file'] ?>">
                                                    <img class="img-fluid" src="assets/merchant_contest_uploads/<?php echo $uploads['upload_file'] ?>" alt="">
                                                </a>

                                            <?php } elseif ($type == 'video') { ?>
                                                <a href="assets/merchant_contest_uploads/<?php echo $uploads['upload_file'] ?>">
                                                    <video class="img-center w-100" controls>
                                                        <source src="assets/merchant_contest_uploads/<?php echo $uploads['upload_file'] ?>">
                                                    </video>
                                                </a>
                                        ...
                                        <div class="product-desc">
                                            <p class="product-name mt-4 mb-2 d-block link-title">بواسطة: <?php echo $uploads['merchant_name'] ?></p>
                                            <?php if (isset($_SESSION['voter_id'])) {
                                                $check_voted = mysqli_query($conn, "SELECT voted,m_contest_id from voter_merchant_contest where voter_id='{$_SESSION['voter_id']}'");

                                            ?>
                                                <div class="product-link mt-3">
                                                    <form action="" method="post">
                                                        <input type="hidden" name="c_id" value='<?php echo $uploads['m_contest_id'] ?>'>
                                                        ...
                                                        while ($vot = mysqli_fetch_assoc($check_voted)) {
                                                            array_push($votes, $vot['m_contest_id']);
                                                            if (in_array($uploads['m_contest_id'], $votes)) {
                                                                $v = 1;
                                                            } else {
                                                                $v = 0;
                                                            }
                                                ...
                                                $check_voted_sponsor = mysqli_query($conn, "SELECT voted,sponsor_id,merchant_contest_id from voters_active_contest where sponsor_id='{$_SESSION['sponsor_id']}'");
                                                    ...
                                                    <div class="product-link mt-3">
                                                        <form action="" method="post">
                                                            ...
                                                            while ($vot = mysqli_fetch_assoc($check_voted_sponsor)) {
                                                                array_push($votes_sponsors, $vot['merchant_contest_id']);
                                                                if (in_array($uploads['m_contest_id'], $votes_sponsors)) {
                                                                    $v = 1;
                                                                } else {
                                                                    $v = 0;
                                                                }
                                                ...
                                                $check_voted_merchants = mysqli_query($conn, "SELECT voted,merchant_id,merchant_contest_id from voters_active_contest where merchant_id='{$_SESSION['merchant_id']}'");
                                                ...
                                                if ($checked['active'] == 1) { ?>
                                                    ...
                                                    <div class="product-link mt-3">
                                                        <form action="" method="post">
                                                            ...
                                                            $checkbelong = array();
                                                            while ($vot = mysqli_fetch_assoc($check_voted_merchants)) {
                                                                array_push($votes_merchants, $vot['merchant_contest_id']);
                                                                if (in_array($uploads['m_contest_id'], $votes_merchants)) {
                                                                    $v = 1;
                                                                } else {
                                                                    $v = 0;
                                                                }
                                                            }
                                                            $check_merchant = mysqli_query($conn, "SELECT upload_file from merchant_contest where merchant_id='{$_SESSION['merchant_id']}'");
                                                            while ($row = mysqli_fetch_assoc($check_merchant)) {
                                                ...
                                                $check_voted_user = mysqli_query($conn, "SELECT voted,user_id,merchant_contest_id from voters_active_contest where user_id='{$_SESSION['user_id']}'");
                                                    ...
                                                    <div class="product-link mt-3">
                                                        <form action="" method="post">
                                                            ...
                                                            while ($vot = mysqli_fetch_assoc($check_voted_user)) {
                                                                array_push($votes_users, $vot['merchant_contest_id']);
                                                                if (in_array($uploads['m_contest_id'], $votes_users)) {
                                                                    $v = 1;
                                                                } else {
                                                                    $v = 0;
                                                                }
